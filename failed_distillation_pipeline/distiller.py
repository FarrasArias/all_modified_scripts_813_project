...
class Distiller:
    def __init__(
        ...
        ####################### My code for dataloader
        #######################
        dataset_path = "./full_dataset_131022_NUM_BARS=4_OPZ_False.arr"
        dataset_dataloader = CustomDataset(split_id=0, is_training=True, dataset=dataset_path)
        self.dataloader = dataset_dataloader
        #######################
        #######################
    ...
    def train(self):
        ...
        for _ in range(self.params.n_epoch):
            ...
            for batch in iter_bar:
                ...
                #logger.info(token_ids, attn_mask, lm_labels)
                token_ids = batch["input_ids"].to(f"cuda:{self.params.local_rank}")
                attn_mask = batch["attention_mask"].to(f"cuda:{self.params.local_rank}")
                lm_labels = batch["labels"].to(f"cuda:{self.params.local_rank}")
                logger.info(token_ids)
                logger.info(attn_mask)
                logger.info(lm_labels)
                logger.info(f"id_3")
